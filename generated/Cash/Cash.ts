...
export class Cash extends ethereum.SmartContract {
  ...
  isOperator(): boolean {
    let result = super.call("isOperator", "isOperator():(bool)", []);

    return result[0].toBoolean();
  }

  try_isOperator(): ethereum.CallResult<boolean> {
    let result = super.tryCall("isOperator", "isOperator():(bool)", []);
    if (result.reverted) {
      return new ethereum.CallResult();
    }
    let value = result.value;
  ...
  mint(recipient_: Address, amount_: BigInt): boolean {
  ...
  try_mint(recipient_: Address, amount_: BigInt): ethereum.CallResult<boolean> {
